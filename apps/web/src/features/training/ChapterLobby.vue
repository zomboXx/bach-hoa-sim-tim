...
import { apiMode, request } from "../../shared/server";
...
async function launch() {
  ...
  try {
    ...
    if (!check.ok || !(await check.text()).includes("Engine"))
      throw Error("Chưa có bản export Godot. Chạy scripts/export-training.ps1 rồi build lại PWA.");
...
async function flush() {
  while (pendingEvents.length && !disposed) {
    ...
    try {
      ...
      error.value = (e as Error).message + " Tiến độ chưa lưu sẽ được thử lại khi em nhấn Lưu lại.";
      return;
    }
  }
}
...
function onMessage(event: MessageEvent) {
  ...
  if (
    ...
    ["welcome", "sensitive", "delivery"].every((k) => typeof payload.answers?.[k] === "string")
  ) {
    pendingEvents.push({
      type: "orientation",
      answers: {
  ...
  if (
    ...
    visited.value.length === 6 &&
    (orientationDone.value || pendingEvents.some((e) => e.type === "orientation")) &&
    !completed.value &&
...
async function leave() {
  await sequence;
  if (pendingEvents.length && !confirm("Tiến độ chưa lưu hết. Em vẫn muốn rời chương?")) return;
  playing.value = false;
  clearTimeout(loadTimer);
  error.value = "";
}
function fullscreen() {
  frame.value?.requestFullscreen?.().catch(() => {
    error.value = "Trình duyệt không hỗ trợ toàn màn hình; thử xoay ngang điện thoại.";
  });
}
onMounted(async () => {
  ...
  if (apiMode)
    try {
      previous.value = (await request<{ status: string }[]>("/training/sessions")).some(
...
<template>
  <div class="academy">
    <header class="academy-header">
      ...
      <a class="academy-brand" href="#" @click.prevent="playing ? leave() : undefined"
        ><img src="/icon.svg" alt="" /> sim tím <span>ACADEMY</span></a
      >
      <span class="academy-employee"
        >{{ employeeName }} <span class="academy-avatar">{{ employeeName[0] }}</span></span
      >
    </header>
    <main v-if="!playing" class="academy-lobby">
      <section class="academy-intro">
        <div>
          ...
          <p>
            Vào vai nhân viên mới, gặp gỡ khách hàng và học cách<br class="desktop-break" />
        ...
        <div class="academy-mentor">
          ...
          <div>
            ...
            <p>
              Không ảnh hưởng đơn hàng hay tồn kho thật.<br />Một không gian an toàn để làm quen.
            </p>
          </div>
        </div>
      </section>
      ...
      <section class="chapter-zero">
        <div class="chapter-zero-copy">
          <span class="academy-tag"
            >CHAPTER 0 <i></i> {{ previous ? "ĐÃ HOÀN THÀNH" : "SẴN SÀNG KHÁM PHÁ" }}</span
          >
          <h2>Ngày đầu tiên<br />tại Sim Tím.</h2>
          <p>
            Mặc đồng phục, bước vào cửa hàng 2D và đi một vòng cùng mentor. Làm quen 6 khu vực trước
            khi bắt đầu ca làm việc đầu tiên.
          </p>
          <div class="chapter-meta">
            <span>◷ 5–8 phút</span><span>⌘ Di chuyển & tương tác</span>
          </div>
          <button class="academy-launch" :disabled="busy" @click="launch">
            {{ busy ? "Đang chuẩn bị…" : previous ? "Khám phá lại cửa hàng" : "Bắt đầu Chapter 0" }}
            <span>↗</span></button
        ...
        <div class="chapter-zero-map">
          ...
          />
          <div class="map-caption"><span class="map-dot"></span> CỬA HÀNG MÔ PHỎNG <b>01</b></div>
      ...
      <footer class="academy-footer">
        Thực hành trong mô phỏng. Tự tin ngoài cửa hàng.<span>SIM TÍM · LEARN BY DOING</span>
      </footer>
    </main>
    <main v-else class="academy-play">
      <div class="game-heading">
        ...
        <button class="academy-back" @click="fullscreen">⛶ Toàn màn hình</button>
      </div>
      ...
      <div class="game-layout">
        <div class="game-stage">
          ...
          <div v-if="!ready" class="game-loading">
            Đang mở cửa hàng 2D…<small>Lần đầu cần tải bản Godot Web. Vui lòng chờ.</small>
          </div>
        </div>
        <aside class="tour-guide">
          <span class="academy-tag">NHIỆM VỤ LÀM QUEN</span>
          <h2>Một vòng cửa hàng</h2>
          <p>Đi tới điểm sáng, nhấn <kbd>E</kbd> và đọc lời hướng dẫn từ mentor.</p>
          <ol>
            <li v-for="(s, i) in stations" :key="s[0]" :class="{ visited: visited.includes(s[0]) }">
          ...
          <p class="game-saved" v-if="completed" role="status">
            ✓ Hoàn thành Chapter 0.<br />{{
              apiMode ? "Kết quả đã lưu trên máy chủ." : "Kết quả chỉ thuộc phiên demo này."
            }}
          </p>
        </aside>
      </div>
      <div class="game-help">
        <span><kbd>W A S D</kbd> / <kbd>↑ ← ↓ →</kbd> Di chuyển &nbsp; <kbd>E</kbd> Tương tác</span
        ><span>Điện thoại: dùng nút trong game · Xoay ngang để nhìn rộng hơn</span>
      </div>
    </main>
