...
import { roleNames, type User, type State, type Count } from "../../shared/api";
...
const revenue = computed(() => props.state.invoices.reduce((s, i) => s + i.total, 0));
const requests = computed(() => props.state.counts.filter((c) => c.status === "REVIEW"));
const roles = [
  [
    "Admin / Chủ cửa hàng",
  ...
  ["Quản lý cửa hàng", "Chốt duyệt hủy hóa đơn, trả/hoàn tiền, chênh lệch kiểm kho và mở/đóng ca."],
  [
    "Sales / Cashier",
    "Lập đơn, quét/nhập mã vạch, tính tiền POS; xem hóa đơn của mình. Không tự duyệt ngoại lệ.",
  ],
  ["Warehouse", "Lập phiếu nhập, xuất, điều chuyển; không bán hàng hoặc phê duyệt điều chỉnh tồn."],
];
</script>
<template>
  <div class="role-space">
    ...
    <template v-if="route === 'finance'">
      <div class="role-metrics">
        <article class="card">
          ...
          <h2>{{ money(revenue) }}</h2>
          <p>{{ state.invoices.length }} hóa đơn · Không phải lợi nhuận hay số dư két</p>
        </article>
        <article class="card">
      ...
      <button class="secondary" @click="$emit('go', 'invoices')">Tra cứu hóa đơn →</button>
      <section class="card scope-note">
        <h2>Phạm vi kế toán đang phát triển</h2>
        <p>
          Thu chi · Công nợ · Đối soát két tiền. Chưa có thao tác ghi sổ trong bản này; không dùng
          doanh số hóa đơn thay cho báo cáo tài chính.
        </p>
        <p>
          Kế toán không được tạo đơn bán, nhận hàng, sửa danh mục, duyệt kiểm kê hoặc can thiệp vận
          hành nhân sự.
        </p>
      </section>
    </template>
    <template v-else-if="route === 'approvals'">
      <section class="card scope-note">
        ...
        <p>
          Chỉ điều chỉnh tồn sau khi người có quyền duyệt. Nếu dữ liệu lô đã thay đổi, phiếu chuyển
          sang xung đột để kiểm lại.
        </p>
        <div v-if="!requests.length" class="empty">Chưa có phiếu cần duyệt.</div>
        <article v-for="c in requests" :key="c.id" class="approval-item">
          <div>
            <b
              >{{ state.products.find((p) => p.id === c.productId)?.name || c.productId }} ·
              {{ c.batchId }}</b
            >
            <p>
        ...
        <button class="secondary" @click="$emit('go', 'count')">Xem toàn bộ phiếu kiểm kê →</button>
      </section>
      <div class="role-metrics">
        <article
          v-for="label in ['Hủy hóa đơn', 'Trả hàng / hoàn tiền', 'Mở / đóng ca']"
          :key="label"
          class="card"
        >
          ...
          <h2>{{ label }}</h2>
          <p>
            Yêu cầu nghiệp vụ đã ghi nhận: quản lý cửa hàng là chốt duyệt. Chưa có API thực thi;
            không tự thay đổi tiền hay chứng từ.
          </p>
        </article>
      </div>
    </template>
    <template v-else-if="route === 'warehouse_orders'">
      <div class="role-metrics">
        <article class="card">
          ...
          <button class="primary" @click="$emit('go', 'receive')">Tạo phiếu nhập →</button>
        </article>
        <article
    ...
    <template v-else>
      ...
      <section class="card scope-note">
        ...
        <p>
          Hiện vận hành một cửa hàng mẫu. Quản lý chuỗi, cấu hình dịch vụ và quản lý tài khoản chưa
          có chức năng ghi. Admin có quyền cao nhất trên các API đã triển khai; không phải SaaS đa
          doanh nghiệp hoàn chỉnh.
        </p>
      </section>
    </template>
    <aside class="role-training">
      ...
      <button class="secondary" @click="$emit('go', 'training')">Học cùng mentor ↗</button>
    </aside>
  </div>
</template>
<style scoped>
