...
import { available, price, type State, type Product } from "../../shared/api";
...
const matches = computed(() => {
  ...
  return term
    ? props.state.products.filter(
        (p) =>
          p.active !== false &&
          `${p.name} ${p.barcode} ${p.id}`.toLocaleLowerCase("vi").includes(term),
      )
    : [];
});
const units = computed(() => props.cart.reduce((sum, row) => sum + row.quantity, 0));
...
function submitSearch() {
  ...
  const p = exact || (matches.value.length === 1 ? matches.value[0] : undefined);
  if (p) choose(p);
  else
...
async function editQuantity(id: string, event: Event) {
  ...
  if (!/^[0-9]+$/.test(input.value) || !Number.isSafeInteger(Number(input.value))) {
...
<template>
  <div class="invoice-editor">
    <div class="page-head">
      ...
      <button class="secondary camera-trigger" @click="cameraDialog?.showModal()">
    ...
    <div class="invoice-layout">
      <section class="card invoice-sheet" aria-label="Hóa đơn đang lập">
        <div class="invoice-search">
          ...
          <form class="search-bar" @submit.prevent="submitSearch">
            ...
            <input
              ...
              "
            />
            <button type="submit" :disabled="busy || !connected || !search.trim()">Thêm ↵</button>
          </form>
          <p class="search-help">
            Nhập mã vạch rồi nhấn Enter, hoặc tra tên và chọn sản phẩm. Mỗi lượt thêm tăng 1.
          </p>
          ...
          >
            ...
            >
              ...
              ><span class="result-detail"
                ><b>{{ p.name }}</b
                ><small
                  >{{ p.barcode }} · Còn {{ available(state, p.id) }} {{ p.unit }}</small
                ></span
            ...
            <p v-else-if="matches.length > 8">
              Hiển thị 8 / {{ matches.length }} kết quả. Nhập cụ thể hơn để tìm nhanh.
            </p>
          </div>
        </div>
        ...
        <div v-if="!cart.length" class="invoice-empty">
          ...
          <p>Tra tên hoặc nhập mã vạch ở phía trên<br />để thêm sản phẩm đầu tiên.</p>
        </div>
        <template v-else>
          <div class="invoice-columns" aria-hidden="true">
            <span>Sản phẩm / Đơn giá</span><span>Số lượng</span><span>Thành tiền</span><span></span>
          </div>
          <div v-for="row in cart" :key="row.id" class="cart-row invoice-line">
            <div class="line-product">
              ...
              ><span>{{ money(unitPrice(row.id)) }} / {{ product(row.id).unit }}</span>
            </div>
            <div class="stepper">
              <button
                :disabled="busy || !connected"
            ...
            <strong class="line-total">{{ money(unitPrice(row.id) * row.quantity) }}</strong>
            <button
              class="remove-line"
              :disabled="busy || !connected"
        ...
        <p class="invoice-footnote">
          Số lượng bằng 0 sẽ xóa sản phẩm khỏi hóa đơn. Hóa đơn chỉ được ghi nhận sau khi thanh
          toán.
        </p>
      </section>
    ...
    >
      ...
      <p>Chức năng này chưa được cập nhật. Hiện tại, hãy nhập mã vạch hoặc tra tên sản phẩm.</p>
      <p>
        Dự kiến: điện thoại đăng nhập cùng tài khoản nhân viên và kết nối với hóa đơn đang lập; mỗi
        lần quét sẽ thêm sản phẩm vào hóa đơn đó.
      </p>
      <p class="payment-note">Bản này chưa mở camera và chưa đồng bộ hóa đơn giữa thiết bị.</p>
      <button class="primary wide" @click="cameraDialog?.close()">Đã hiểu</button>
    </dialog>
  </div>
</template>
