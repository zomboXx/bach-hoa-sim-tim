...
import { adjacent, blocked, pathTo, stations, type Point, type Station } from "./world";
defineProps<{ employeeName: string }>();
const emit = defineEmits<{ exit: [] }>();
type Stage = "greet" | "milk" | "checkout" | "cooler" | "quarantine" | "mentor" | "complete";
type Choice = {
  text: string;
  right: boolean;
  feedback: string;
...
> = {
  ...
  quarantine: {
    ...
    detail: "Mang hộp sữa chua hết hạn tới khu hàng cần xử lý, tách khỏi hàng bán.",
...
const locked = computed(
  () => intro.value || !!dialogue.value || exitPrompt.value || stage.value === "complete",
);
...
function interact(s: Station) {
  ...
  if (s.id !== goal.value.station) {
    void converse({
      ...
      choices: [{ text: "Mình hiểu rồi", right: true, feedback: goal.value.detail }],
    });
    return;
  }
    ...
    void converse({
      ...
      choices: [
        ...
        },
        {
          ...
          effect: () => advance("milk", "Đã lắng nghe và xác nhận nhu cầu của khách."),
        },
        {
    ...
    void converse({
      ...
      choices: [
        ...
        },
        {
          ...
          right: false,
          feedback: "Mai: Loại này có đường, chưa đúng nhu cầu khách vừa xác nhận.",
        },
        {
          text: "Lấy 1 hộp sữa ít đường",
          right: false,
          feedback: "Mai: Khách cần hai hộp. Hãy kiểm tra cả loại hàng và số lượng nhé.",
        },
      ],
    });
    ...
    void converse({
      ...
      choices: [
        ...
        },
        {
          ...
          effect: () => {
            bag.expired = 1;
            advance("quarantine", "Đã nhận diện và lấy hàng hết hạn khỏi tủ mát.");
          },
        },
        {
      ...
      choices: [
        {
          ...
          effect: () => advance("complete", "Đã báo cáo đầy đủ cho người phụ trách."),
        },
        {
    ...
    choices: [
      {
        ...
        right: false,
        feedback: "Mai: Hãy tính lại: 20.000 − 17.000 = 3.000 đồng. Đếm lại trước khi đưa khách.",
      },
      {
        ...
        effect: () => {
          bag.milk = 0;
          advance("cooler", "Đã quét đủ hàng, trả đúng 3.000đ và cảm ơn khách.");
        },
      },
      {
...
function keydown(e: KeyboardEvent) {
  if (e.key === "Tab" && (intro.value || exitPrompt.value || stage.value === "complete")) {
    const buttons = [
      ...gameRoot.value!.querySelectorAll<HTMLButtonElement>(".game-overlay button"),
    ];
...
<template>
  <section ref="gameRoot" class="training-game" @keydown="keydown">
    <header class="game-header" :inert="intro || exitPrompt || stage === 'complete'">
      <button @click="leave" class="game-exit">← <span>Về cửa hàng</span></button>
    ...
    <div class="game-content" :inert="intro || exitPrompt || stage === 'complete'">
      ...
      <div class="game-layout">
        <div class="world-column">
          <div class="world-topline">
            <span><i></i> CỬA HÀNG SIM TÍM</span><span>↗ Nhấp vào nơi muốn đến</span>
          </div>
          <div
            ref="world"
            tabindex="0"
          ...
          >
            ...
            >
              <defs>
                <pattern id="floorPattern" width="40" height="40" patternUnits="userSpaceOnUse">
                  ...
                  <path d="M0 40V0H40" fill="none" stroke="#d5c6b4" stroke-width="1" />
                  <path d="M6 6h2v2H6" fill="#d8cbb8" />
                </pattern>
                <pattern id="rugPattern" width="10" height="10" patternUnits="userSpaceOnUse">
                  <rect width="10" height="10" fill="#96749f" />
                  <path d="M0 0h10v1H0" fill="#a48aab" />
                </pattern>
              </defs>
              <rect width="800" height="560" fill="#614b70" />
              <rect x="18" y="65" width="764" height="480" rx="3" fill="#4a3b53" />
              <rect x="30" y="74" width="740" height="456" fill="url(#floorPattern)" />
              ...
              <rect
                ...
                stroke-width="3"
              />
              <text x="130" y="42" fill="#f7e3bb" text-anchor="middle" class="world-store-sign">
                SIM TÍM
              </text>
              <text x="480" y="42" text-anchor="middle" fill="#d8c4e1" class="world-wall-copy">
                MỖI NGÀY MỘT CHÚT TỰ TIN HƠN
              </text>
              <g transform="translate(708 13)">
                <circle cx="18" cy="20" r="17" fill="#f2e6ca" stroke="#b599ba" stroke-width="3" />
                <path d="M18 10v10l8 5" stroke="#806484" stroke-width="2" fill="none" />
              </g>
              <rect x="555" y="434" width="160" height="91" fill="url(#rugPattern)" />
              <rect x="555" y="523" width="160" height="17" fill="#c5b5c7" />
              <text x="635" y="498" text-anchor="middle" fill="#e9d8eb" class="world-floor-label">
                CHÀO MỪNG BẠN
              </text>
              <path
              ...
              />
              <g
                v-for="s in stations.filter((s) => !['mentor', 'customer'].includes(s.id))"
                :key="s.id"
              ...
              >
                ...
                <template v-if="s.type === 'shelf'">
                  ...
                  <rect x="4" y="3" :width="s.w * 40 - 8" :height="s.h * 40 - 9" fill="#d0b18a" />
                  <rect x="8" y="5" :width="s.w * 40 - 16" height="64" fill="#d8c69e" />
                  <g v-for="r in 2" :key="r" :transform="`translate(12,${r === 1 ? 6 : 39})`">
                    <g v-for="n in 6" :key="n" :transform="`translate(${(n - 1) * 16},0)`">
                      <rect
                        width="11"
                        height="24"
                        :fill="
                        ...
                        "
                      />
                      <path v-if="s.id === 'milk'" d="M2 0h7v-3H2z" fill="#709caf" />
                      <rect x="2" y="9" width="7" height="8" fill="#fff7d4" opacity=".7" />
                    </g>
                  </g>
                  <path d="M4 35h112M4 69h112" stroke="#947b67" stroke-width="6" />
                ...
                <template v-if="s.type === 'cooler'">
                  ...
                  <g v-for="n in 7" :key="n" :transform="`translate(${n * 18 - 5},29)`">
                    <path
                      d="M0 0h12l-2 21H2z"
                      :fill="
                        n === 3 &&
                  ...
                  <path d="M77 5v58M6 64h148" stroke="#7d9da3" stroke-width="4" />
                ...
                <template v-if="s.type === 'checkout'">
                  ...
                  <rect x="95" y="13" width="49" height="36" rx="3" fill="#8c8885" />
                  <path d="M101 27h34" stroke="#d78b7b" stroke-width="3" />
                  <rect x="9" y="65" width="139" height="11" fill="#a98778" />
                  <text x="108" y="75" class="shelf-price" fill="#fff3d6">THU NGÂN</text>
                </template>
                ...
                <text :x="s.w * 20" y="-15" text-anchor="middle" class="station-name">
                  {{ s.name }}
                </text>
                <g v-if="goal.station === s.id && !intro" :transform="`translate(${s.w * 20},-41)`">
                  <path d="M-10-11h20v19H3l-3 5-3-5h-7z" fill="#f2bc73" />
                  <text text-anchor="middle" y="3" fill="#664d52" font-size="13" font-weight="bold">
                    !
                  </text>
                </g>
              </g>
              <g
                ...
                @keydown.space.prevent.stop="walkTo(s)"
              >
                <rect x="-23" y="-36" width="86" height="90" fill="transparent" />
                <ellipse
                  class="npc-ring"
                  cx="20"
                  cy="35"
                ...
                <g v-if="goal.station === s.id && !intro" transform="translate(20,-39)">
                  <path d="M-11-14h22v22H3L0 13-3 8h-8z" fill="#ffce86" />
                  <text y="3" text-anchor="middle" fill="#815a45" font-size="17" font-weight="bold">
                    !
                  </text>
                </g>
              </g>
              ...
              >
                ...
                <g v-if="bag.milk || bag.expired" transform="translate(32,10)">
                  <rect
                    ...
                    stroke-width="2"
                  />
                  <path d="M3 0v-5h7v5" fill="none" stroke="#86654f" stroke-width="2" />
                </g>
                <text x="20" y="53" text-anchor="middle" class="player-name">BẠN</text>
              </g>
              <g transform="translate(43,440)">
                <rect x="12" y="18" width="26" height="36" fill="#ae7c5d" />
                <path d="M12 19V-5H0v-16h18V-8h8v-30h16v23h10V2H35v17z" fill="#7c9b74" />
            ...
            >
              ...
              ><span v-else-if="nearby"
                ><kbd>E</kbd>
                {{ nearby.id === goal.station ? goal.verb : "Tương tác với " + nearby.name }}</span
          ...
          <div class="game-controls">
            <div class="keyboard-legend">
              <span><kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> Di chuyển</span
        ...
        <aside class="quest-panel">
          ...
          <div
            ...
            :key="q[0]"
            :class="['quest-item', { done: completed > i, current: goal.chapter === i }]"
          >
            <span>{{ completed > i ? "✓" : String(i + 1).padStart(2, "0") }}</span>
            <div>
              <b>{{ q[0] }}</b
              ><small>{{ q[1] }}</small>
            </div>
          ...
          <div class="mentor-note">
            ...
            <p>“Cứ thử nhé! Nếu làm chưa đúng, chị sẽ hướng dẫn em ngay.”<b>— Mentor Mai</b></p>
          </div>
        </aside>
      </div>
      <div v-if="dialogue" class="conversation" role="region" aria-label="Hội thoại trong trò chơi">
        <div class="conversation-avatar">
          <img v-if="dialogue.speaker === 'Mentor Mai'" src="/mentor.png" alt="Mentor Mai" /><svg
            v-else
            viewBox="-5 -27 50 70"
          >
            <PixelPerson :kind="dialogue.speaker === 'Khách Linh' ? 'customer' : 'player'" />
          </svg>
        </div>
        <div class="conversation-content">
          ...
          <template v-if="dialogue.type === 'scan'"
            ><div class="scan-items">
              ...
              >
                <span>▣</span>{{ scans >= n ? "✓ Đã quét" : "Quét hộp sữa " + n }}<b>8.500đ</b>
              </button>
            </div>
            <div class="scan-total">
              <span>{{ scans }} / 2 sản phẩm</span
              ><b>{{ (scans * 8500).toLocaleString("vi-VN") }}đ</b
              ><button :disabled="scans !== 2" @click="tender">Nhận tiền từ khách →</button>
            </div></template
          >
      ...
      <p class="game-footnote">
        Bản chơi thử 2D trên trình duyệt · Dữ liệu phiên riêng · Tích hợp Godot và lưu kết quả qua
        backend ở giai đoạn sau.
      </p>
    </div>
    <div v-if="intro" class="game-overlay">
      <section class="shift-intro" role="dialog" aria-modal="true" aria-labelledby="intro-heading">
        ...
        <p>
          Đi gặp khách, lấy hàng và thanh toán tại quầy.<br />Cuối ca, cùng Mai xử lý một tình huống
          trên kệ.
        </p>
        <div class="intro-skills">
          <span>♡ Giao tiếp</span><span>▤ Bán hàng</span><span>▦ Hàng hóa</span>
        </div>
        <button class="start-shift" @click="start">Bắt đầu ca thực hành <span>→</span></button
        ><small
          >Di chuyển: WASD / phím mũi tên · Tương tác: E<br />Hoặc nhấp / chạm vào khách và đồ vật
          để đi tới.</small
        ><button class="intro-back" @click="emit('exit')">Về giao diện quản lý</button>
      </section>
    </div>
    <div v-if="exitPrompt" class="game-overlay">
      <section class="leave-dialog" role="dialog" aria-modal="true" aria-labelledby="exit-heading">
        <h2 id="exit-heading">Rời ca thực hành?</h2>
        <p>Tiến độ của lần chơi này sẽ được đặt lại. Dữ liệu cửa hàng không bị ảnh hưởng.</p>
    ...
    <div v-if="stage === 'complete'" class="game-overlay">
      ...
      >
        ...
        <p>Bạn đã trực tiếp chăm khách, bán hàng và xử lý hàng trên kệ trong cửa hàng 2D.</p>
        ...
        <p class="practice-summary">
          {{ steps }} bước chân ·
          {{
            errors ? `${errors} lần được mentor hướng dẫn lại` : "Hoàn thành đúng ngay từ lần đầu"
          }}
        </p>
        <div class="complete-history">
          <p v-for="entry in [...log].reverse()" :key="entry">✓ {{ entry }}</p>
        </div>
        <button class="start-shift" @click="emit('exit')">Trở về cửa hàng →</button
