...
test("touch controls stop on release; exit and re-entry reset the practice session", async ({
  ...
  await page.getByRole("button", { name: "Mở đào tạo cùng Mentor Mai", exact: true }).click();
  await page.getByRole("button", { name: /Bắt đầu ca thực hành/ }).click();
  const control = await page.getByRole("button", { name: "Đi phải", exact: true }).boundingBox();
  await page.mouse.move(control!.x + control!.width / 2, control!.y + control!.height / 2);
  await page.mouse.down();
  await expect(page.locator(".world-player")).not.toHaveAttribute("data-x", "10");
  await page.mouse.up();
  ...
  await expect(page.getByRole("heading", { name: "Rời ca thực hành?" })).toBeVisible();
  ...
  await expect(page.locator(".world-player")).toHaveAttribute("data-stage", "greet");
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
});

async function playShift(page: Page) {
  await page.getByRole("button", { name: "Bắt đầu ca thực hành", exact: false }).click();
  ...
  await page.getByRole("button", { name: /Chị cứ lấy loại đang giảm giá/ }).click();
  await expect(page.locator(".choice-feedback")).toContainText("chưa chắc phù hợp");
  await expect(game).toHaveAttribute("data-stage", "greet");
  await page.getByRole("button", { name: /Dạ, chị muốn 2 hộp sữa ít đường/ }).click();
  await page.getByRole("button", { name: /Tiếp tục trong cửa hàng/ }).click();
  await page.getByRole("button", { name: "Đi đến Kệ sữa & đồ uống", exact: true }).click();
  await page.getByRole("button", { name: /Lấy 2 hộp sữa ít đường/ }).click();
  await page.getByRole("button", { name: /Tiếp tục trong cửa hàng/ }).click();
  await expect(page.locator(".bag-panel")).toContainText("× 2");
  await page.getByRole("button", { name: "Đi đến Quầy thanh toán", exact: true }).click();
  await page.getByRole("button", { name: "Quét hộp sữa 1", exact: true }).click();
  await expect(page.getByRole("button", { name: /Nhận tiền từ khách/ })).toBeDisabled();
  await page.getByRole("button", { name: "Quét hộp sữa 2", exact: true }).click();
  ...
  await page.getByRole("button", { name: /Em gửi chị 3.000đ tiền thừa/ }).click();
  await page.getByRole("button", { name: /Tiếp tục trong cửa hàng/ }).click();
  await page.getByRole("button", { name: "Đi đến Tủ mát", exact: true }).click();
  await page.getByRole("button", { name: /Giảm giá để bán nhanh/ }).click();
  await expect(page.locator(".choice-feedback")).toContainText("không được tiếp tục bán");
  ...
  await page.getByRole("button", { name: "Đi đến Khu hàng cần xử lý", exact: true }).click();
  await page.getByRole("button", { name: /Đặt vào khay riêng/ }).click();
  await page.getByRole("button", { name: /Tiếp tục trong cửa hàng/ }).click();
  await page.getByRole("button", { name: "Đi đến Mentor Mai", exact: true }).click();
  await page.getByRole("button", { name: /Em đã bán đúng hàng cho khách/ }).click();
  await page.getByRole("button", { name: /Kết thúc ca/ }).click();
  await expect(page.getByRole("heading", { name: "Một ca làm, ba kỹ năng." })).toBeVisible();
  await expect(page.locator(".complete-skills")).toContainText("Giao tiếp & tư vấn");
  await expect(page.locator(".complete-skills")).toContainText("Thanh toán & tiền thừa");
  await expect(page.locator(".complete-skills")).toContainText("Hàng hết hạn & bàn giao");
}
...
test("receive → sell → invoice → offline count → approval; training isolation", async ({
  ...
  await nav.getByRole("button", { name: "Tồn kho & lô hàng", exact: false }).click();
  await expect(page.locator("tr").filter({ hasText: "LO01" })).toContainText("47");
  await expect(page.locator("tr").filter({ hasText: "LO07" })).toContainText("6");
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  await page.getByLabel("Ghi chú", { exact: true }).fill("Kiểm kê offline tại kệ");
  await page.getByRole("button", { name: "Lưu phiếu offline" }).click();
  await expect(page.locator(".count-row")).toContainText("Chờ kết nối");
  await page.reload();
  await page.getByRole("button", { name: "Mở menu", exact: true }).click();
  await nav.getByRole("button", { name: "Kiểm kê", exact: false }).click();
  await expect(page.locator(".count-row")).toContainText("Kiểm kê offline tại kệ");
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  ...
  await page.getByRole("button", { name: "Mở menu", exact: true }).click();
  await nav.getByRole("button", { name: "Đào tạo nghiệp vụ", exact: false }).click();
  await playShift(page);
  await page.getByRole("button", { name: "Trở về cửa hàng", exact: false }).click();
  await page.getByRole("button", { name: "Mở menu", exact: true }).click();
  await nav.getByRole("button", { name: "Tồn kho & lô hàng", exact: false }).click();
...
test("demo login and sales role hide managerial controls", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Vào không gian làm việc" }).click();
  await expect(
    page.locator("nav").getByRole("button", { name: /Nhận hàng|Báo cáo|Khuyến mãi|Kiểm kê/ }),
  ).toHaveCount(0);
  await page.locator("nav").getByRole("button", { name: "Sản phẩm", exact: false }).click();
  await expect(page.getByRole("button", { name: "Thêm sản phẩm", exact: false })).toHaveCount(0);
