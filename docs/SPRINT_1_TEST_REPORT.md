...
Ảnh tự động: `prototype-v2/test-results/academy-desktop.png`, `academy-mobile.png`, `godot-mentor.png`, `godot-mobile.png`. Đây là output tạm bị ignore, sẽ được CI upload làm artifact. Sprite alpha thật đã kiểm tra trong game Web. Godot tour chạy riêng ở `godot/tests/tour_test.gd`; completion/orientation API kiểm thử độc lập bằng MockMvc, chưa có test browser tự chơi toàn bộ 6 điểm và 3 câu từ đầu đến cuối.
