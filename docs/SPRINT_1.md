...
Chi tiết các chức năng còn thiếu và chốt quyền: [ROLES_AND_CHAPTER_0.md](ROLES_AND_CHAPTER_0.md).
...
2. `mvn -f backend/pom.xml spring-boot:run`. Flyway tự tạo schema; seed 5 tài khoản mẫu, bổ sung tài khoản còn thiếu nhưng không ghi đè mật khẩu đã có. Hàng mẫu chỉ seed lần đầu. API mặc định localhost:8080. `GET http://127.0.0.1:8080/api/health` phải trả `UP`.
3. `pwsh -File scripts/export-training.ps1 -Godot "đường-dẫn-godot-console.exe"`. Linux: `godot --headless --path godot --editor --import` rồi `godot --headless --path godot --export-release Web` (tạo `prototype-v2/public/training` trước).
4. `cd prototype-v2`, `npm ci`, `npm run dev`, mở `http://127.0.0.1:5174`.
...
Tạo DB test riêng: `createdb ... simtim_test` (không dùng DB vận hành). `mvn -f backend/pom.xml test`; có thể đặt `TEST_DB_URL` và DB credentials qua môi trường. Test dùng PostgreSQL thật, migration thật, Spring Security/MockMvc, rollback sau mỗi test chức năng.

`npm run build` rồi `npm run test:e2e` trong `prototype-v2`; API và Godot export phải sẵn sàng. `E2E_URL` cho phép trỏ dev server khi kiểm thử; mặc định preview:4174. Các test legacy `workflows.spec.ts` thuộc prototype SVG cũ, không phải suite nghiệm thu bản Godot hiện tại.

`godot --headless --path godot --script res://tests/tour_test.gd` thực sự di chuyển nhân vật qua bản đồ có collision tới đủ 6 điểm; không teleport, không thêm API debug vào game Web.
...
| DB-01 | `docs/DATA_MODEL.md`, Flyway V1/V2/V3 | Review ERD/data dictionary |
| BE-01 | `backend`, `compose.yaml`; health và migration trên PostgreSQL thật | Kiểm tra máy các thành viên |
