...
export function login(id: string, password: string): User {
  ...
  if (!user || password !== "demo123")
    throw Error("Mã nhân viên hoặc mật khẩu chưa đúng.");
  return user;
}
...
export function price(s: State, p: Product) {
  const promo = s.promotions.find(
    (x) => x.productId === p.id && x.end >= today(),
  );
...
export function checkout(
  ...
  if (!permissions[user.role].includes("sale"))
    throw Error("Tài khoản không có quyền bán hàng.");
  if (!lines.length) throw Error("Thêm sản phẩm vào giỏ trước khi thanh toán.");
  for (const l of lines)
    if (
      !Number.isInteger(l.quantity) ||
      l.quantity < 1 ||
      available(s, l.id) < l.quantity
    )
