  ...
  test(`invoice-first editor: search, digits-only quantity, zero removal, camera placeholder (${mobile ? "mobile" : "desktop"})`, async ({
    ...
    const product = state.products.find(
      (p: { id: string; active: boolean }) =>
        ...
        state.batches
          .filter(
            (b: { productId: string; expiry: string }) =>
              b.productId === p.id && b.expiry >= today,
          )
          .reduce(
    ...
    await expect(row).toHaveCount(1);
    await row
      .getByRole("button", { name: "Tăng " + product.name, exact: true })
      .click();
    ...
    await row
      .getByRole("button", { name: "Giảm " + product.name, exact: true })
      .click();
    await expect(quantity).toHaveValue("3");
    const adjustedPrice = state.promotions.find(
      (p: { productId: string; end: string }) =>
        p.productId === product.id && p.end >= today,
    );
    ...
    await page
      .getByRole("button", { name: "Thêm " + product.name, exact: true })
      .click();
    await expect(quantity).toHaveValue("1");
    await row
      .getByRole("button", { name: "Giảm " + product.name, exact: true })
    ...
    await expect(row).toHaveCount(0);
    await page
      .getByRole("button", { name: "Quét mã bằng camera", exact: false })
      .click();
    await expect(page.getByRole("dialog")).toContainText(
      "chưa mở camera và chưa đồng bộ hóa đơn",
    );
    await page.getByRole("button", { name: "Đã hiểu", exact: true }).click();
    await expect(page.getByRole("dialog")).not.toBeVisible();
  });
}
test("server login, sales permissions, sale persists after reload", async ({
  page,
}) => {
  await login(page);
  await expect(
    page
      .locator("nav")
      .getByRole("button", { name: "Nhận hàng", exact: false }),
  ).toHaveCount(0);
  await page
    .locator("nav")
    .getByRole("button", { name: "Bán hàng", exact: false })
    .click();
  const catalog = await (await page.request.get("/api/products")).json();
  const stocked = catalog.find(
    (p: { barcode: string; active: boolean }) => p.active && p.barcode,
  );
  ...
  await expect(page.locator(".invoice-row").first()).toContainText(
    "Lan Nguyễn",
  );
  await page.reload();
  await page
    .locator("nav")
    .getByRole("button", { name: "Hóa đơn", exact: false })
    .click();
  await expect(page.locator(".invoice-row").first()).toContainText(
    "Lan Nguyễn",
  );
...
test("chapter lobby, all six placeholders, actual Godot launch and mentor interaction", async ({
  ...
  await page
    .getByRole("button", { name: /Bắt đầu Chapter 0|Khám phá lại cửa hàng/ })
    .click();
...
test("manager catalog CRUD and stock receiving persist through the API", async ({
  page,
}) => {
  ...
  await page
    ...
    .click();
  await page
    .getByRole("button", { name: "Thêm danh mục", exact: false })
    .click();
  await page.getByLabel("Tên danh mục").fill(category);
  await page.getByRole("button", { name: "Lưu thay đổi" }).click();
  await expect(page.getByRole("dialog")).not.toBeVisible();
  await page
    .getByRole("button", { name: "Thêm sản phẩm", exact: false })
    .click();
  ...
  await page
    .locator("nav")
    .getByRole("button", { name: "Nhà cung cấp", exact: false })
    .click();
  await page
    .getByRole("button", { name: "Thêm nhà cung cấp", exact: false })
    .click();
  ...
  await page
    .getByLabel("Sản phẩm", { exact: true })
  ...
  await page
    .getByRole("button", { name: "Xác nhận nhận", exact: false })
    .click();
  await expect(
    page.locator(".receipt-row").filter({ hasText: product }),
  ).toContainText("Đã nhận 9");
  await page
    .locator("nav")
    .getByRole("button", { name: "Tồn kho & lô hàng", exact: false })
    .click();
  await expect(
    page.locator("tbody tr").filter({ hasText: "E2E-" + suffix }),
  ).toContainText("9");
});
...
test("manager offline count queue survives reload, syncs once, and requires explicit approval", async ({
  ...
  await page
    .locator("nav")
    .getByRole("button", { name: "Kiểm kê", exact: false })
    .click();
  ...
  await page
    .getByRole("button", { name: "Lưu phiếu offline", exact: false })
    .click();
  await expect(
    page.locator(".count-row").filter({ hasText: note }),
  ).toContainText("Chờ kết nối");
  await page.reload();
...
for (const [id, allowed, forbidden] of [
  ...
  [
    "KT001",
    "Không gian kế toán",
    ["Bán hàng", "Nhận hàng", "Kiểm kê", "Trung tâm phê duyệt"],
  ],
  [
    "QL001",
    "Trung tâm phê duyệt",
    ["Bán hàng", "Nhận hàng", "Không gian kế toán"],
  ],
  ...
  test(`workspace boundaries and mentor return: ${id}`, async ({ page }) => {
    ...
    await page.locator(".mentor-fab").click();
    await page
      .getByRole("button", { name: "Về cửa hàng", exact: false })
      .click();
